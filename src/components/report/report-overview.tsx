...
export function ReportOverview({ report }: { report: ReportResponse }) {
  const [cycleSource, setCycleSource] = useState<CycleSource>("bazi");
  ...
  return (
    <section id="overview" className="scroll-mt-20 space-y-5">
      <div className="rounded-md border border-fs-line bg-white p-5 md:p-6">
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="space-y-3">
            <div className="inline-flex rounded-md border border-fs-line bg-fs-surface p-1">
              {(["bazi", "ziwei"] as const).map((source) => (
                <button
                  key={source}
                  type="button"
                  onClick={() => setCycleSource(source)}
                  className={`rounded px-3 py-1.5 text-sm font-medium transition ${
                    cycleSource === source ? "bg-fs-ink text-white" : "text-fs-muted hover:bg-white hover:text-fs-ink"
                  }`}
                >
                  {source === "bazi" ? "八字" : "紫微"}
                </button>
            ...
            <CurrentCycleCard report={report} source={cycleSource} />
          </div>
        </div>
        <div className="mt-5">
      ...
      <div id="chart" className="scroll-mt-24">
        {cycleSource === "bazi" ? <BaziElementBoard report={report} /> : <ZiweiPalaceBoard report={report} />}
      </div>
    </section>
  );
}
